...
const onSubmitContractDeploy = async (event) => {
    ...
    console.log({ bytecode: event.currentTarget.bytecode.value });

    // Encode function and value to append to bytecode
    const encodedFunctionValue = ethers.utils.defaultAbiCoder.encode(["string"], [`${event.currentTarget.bytecode.value}`]).slice(2);
    console.log({ encodedFunctionValue });

    // Reset & Set Loading State
    const codeContractDeploy = document.getElementById('code-contract-deploy');
    codeContractDeploy.innerHTML = '(Loading...)';
    ...
    try {
        ...
        codeContractDeploy.innerHTML = `https://mumbai.polygonscan.com/tx/${result}`;
    } catch (error) {
        console.log({ error });
        codeContractDeploy.innerHTML = error?.message ?? 'Unknown contract deploy error.';
    }
 ...
 */
window.onload = () => {
    // Set bytecode for textarea
    const textareaBytecode = document.querySelector('#form-contract-deploy textarea');
    textareaBytecode.value = CONTRACT_BYTECODE;
